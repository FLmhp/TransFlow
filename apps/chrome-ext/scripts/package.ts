 ...
 *  2. Zip the dist/ directory into dist/transflow-chrome.zip — this zip
...
const STATIC_COPIES: { from: string; to: string }[] = [
  ...
  // PDF viewer static assets — the viewer JS bundle is emitted by tsdown.
  {
  ...
  // pdf.js web worker — must live next to viewer.js so `workerSrc` can
  // resolve it with a relative URL on the extension origin.
  {
...
async function zipExtension(): Promise<void> {
  const zipPath = join(DIST, "transflow-chrome.zip");
  ...
  await new Promise<void>((resolvePromise, rejectPromise) => {
    ...
    archive.glob("**/*", {
      cwd: DIST,
      ignore: ["transflow-chrome.zip", "**/*.map"],
      dot: false,
    });
  ...
  console.log(`✓ Packaged extension → dist/transflow-chrome.zip (${kb} KB)`);
}
