...
- 📄 **PDF 翻译** —— 任意 `.pdf` 链接都可在内置 PDF.js 查看器中打开并逐行翻译；在设置中打开"自动用 TransFlow 查看器打开 .pdf 链接"可实现自动重定向。Tampermonkey 版本只能在页面自带 PDF.js 查看器（例如 Firefox 内置查看器）的场景下翻译 PDF——用户脚本无法提供自己的扩展页面，直接翻译 `.pdf` URL 请使用浏览器扩展版本。
